...
function readWorkerParams {
  ...
  if [ -z "${SOURCE_ARN}" ]; then
    ...
    SOURCE_NAME=$( echo "$SOURCE_ARN" | sed -E -n 's#^arn:aws-cn:kinesis:[a-z]+-[a-z]+-[0-9]:[0-9]{12}:stream/([a-zA-Z0-9_-]{1,128})$#\1#p' )
    if [ ! -z "$SOURCE_NAME" ]; then
      SOURCE_TYPE=kinesis
    else
      SOURCE_NAME=$( echo "$SOURCE_ARN" | sed -E -n 's#^arn:aws-cn:dynamodb:[a-z]+-[a-z]+-[0-9]:[0-9]{12}:table/([a-zA-Z0-9._-]{2,255})$#\1#p' )
      if [ ! -z "$SOURCE_NAME" ]; then
